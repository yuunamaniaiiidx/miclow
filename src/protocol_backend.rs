...
impl ProtocolBackend {
    fn parse_system_control_command_from_outcome(topic: &str, data: &str) -> Option<ExecutorEvent> {
    ...
    fn parse_return_message_from_outcome(topic: &str, data: &str) -> Option<ExecutorEvent> {
        let topic_lower = topic.to_lowercase();
        let data_trimmed = data.trim();
        ...
        let process_stream_outcome = |outcome: Result<StreamOutcome, String>, line_content: &str| {
            match outcome {
                Ok(StreamOutcome::Emit { topic, data }) => {
                    if let Some(return_message_event) = ProtocolBackend::parse_return_message_from_outcome(&topic, &data) {
                        let _ = event_tx.send(return_message_event);
                    } else if let Some(system_control_cmd_event) = ProtocolBackend::parse_system_control_command_from_outcome(&topic, &data) {
